        ...
        <div className="relative min-h-screen bg-[#050505] text-white font-sans flex flex-col selection:bg-blue-500/30 overflow-x-hidden">
            ...
            <header className="fixed top-0 left-0 right-0 z-[100] border-b border-white/5 bg-black/40 backdrop-blur-xl">
                <div className="max-w-6xl mx-auto px-6 h-20 flex justify-between items-center">
                    <h2 className="text-lg font-medium tracking-tight bg-gradient-to-r from-white to-gray-500 bg-clip-text text-transparent cursor-pointer" onClick={() => setView('list')}>
                        Kirill Myakotin
                    </h2>
                    ...
                    <nav className="flex gap-2">
                        <button onClick={() => setView('list')} className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${view === 'list' ? 'bg-white text-black' : 'hover:bg-white/10'}`}>Статьи</button>
                        <button onClick={() => { setEditingArticle(undefined); setView('create'); }} className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${view === 'create' ? 'bg-blue-600 text-white shadow-[0_0_20px_rgba(37,99,235,0.4)]' : 'bg-white/5 hover:bg-white/10'}`}>+ Создать</button>
                    </nav>
                </div>
            </header>
            ...
            <main className="relative z-10 flex-grow pt-32 pb-20 max-w-5xl mx-auto px-6 w-full">
                ...
                {view === 'detail' && selectedArticle && (
                    <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000">
                        <button onClick={() => setView('list')} className="flex items-center gap-2 text-gray-500 hover:text-white mb-12 transition-colors group">
                ...
                {view === 'create' && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                        <ArticleForm article={editingArticle} onSave={fetchArticles} onCancel={() => setView('list')} />
                    </div>
                )}
            </main>
